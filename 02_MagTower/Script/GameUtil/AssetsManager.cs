...
public class AssetsManager : MonoBehaviour
{
    ...
    public void PlayOneShot(string key)
    {
        ...
        {
            // ボリューム調整するキーを設定するゴロ
            case GameUtil.Const.SOUND_KEY_MAGNE_PLUS:
                volume = 1.0f;
                break;
            case GameUtil.Const.SOUND_KEY_MAGNE_MINUS:
                volume = 0.5f;
                break;
            case GameUtil.Const.SOUND_KEY_CLICK:
            case GameUtil.Const.SOUND_KEY_LEVELUP:
                volume = 0.7f;
                break;
        }
        // 効果音再生
    ...
    public void PlayFirstBGM()
    {
        switch (SceneManager.GetActiveScene().name)
        {
            ...
            case GameUtil.Const.SCENE_NAME_TITLE:
                PlayBGM(GameUtil.Const.BGM_KEY_MAGTHEMA, true);
                break;
            // ゲームシーン
            case GameUtil.Const.SCENE_NAME_GAME:
                PlayBGM(GameUtil.Const.BGM_KEY_MAGTECH, true);
                break;
        }
    }
